defmodule Sorcery.Helpers.Complete do
  ...
  def run(exp) do
    ...
    # Add the token type to the end of the this tuple to find out if it is a
    # function or a module
    {status, result, list, token_type } = expand(code |> Enum.reverse)

    case { status, result, list } do
      { :no, _, _ }  -> {'', ''}
      { :yes, [], _ } -> insert_expression(list, exp, token_type)
      { :yes, _,  _ } -> run(code ++ result)
    end
  end

  def insert_expression(list, expr, token_type) do
    tokens = List.insert_at(list, 0, expr)
    {tokens, token_type}
  end
  ...
  def expand('') do
    expand_import("") |> Tuple.append(:function)
  end

  def expand([h|t]=expr) do
    cond do
      h === ?. and t != []->
        reduce(t) |> expand_dot() |> Tuple.append(:function)
      h === ?: ->
        expand_erlang_modules() |> Tuple.append(:function)
      identifier?(h) ->
        reduce(expr) |> expand_expr() |> Tuple.append(:module)
      (h == ?/) and t != [] and identifier?(hd(t)) ->
        reduce(t) |> expand_expr() |> Tuple.append(:function)
      h in '([{' ->
        expand('')
      true ->
        no()
