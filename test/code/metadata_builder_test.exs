defmodule Sorcery.Code.MetadataBuilderTest do

  use ExUnit.Case

  alias Sorcery.Code.MetadataBuilder  

  test "should grab the first line in a file from metadata" do    
    assert get_subject_definition_line(Sorcery.API.Ping, nil, nil) =~ "defmodule Sorcery.API.Ping do"
  end

  defp get_subject_definition_line(module, func, arity) do    
    {:ok, current_directory} = File.cwd()
    file = current_directory <> "/lib/api/ping.ex"
  ...
  test "behaviours" do
    ...
    assert get_line_behaviours(state, 5)  == [SomeModule.SomeBehaviour]
    assert get_line_behaviours(state, 8)  == []
    assert get_line_behaviours(state, 12)  == [SomeOtherBehaviour]
    assert get_line_behaviours(state, 15)  == []
    assert get_line_behaviours(state, 17)  == [SomeModule.SomeBehaviour]
  end
